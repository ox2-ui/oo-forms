ooForms = {}
ooForms.updateAlerts = function (id, query) {
  console.log('%c id   ',  'background: #B3CC57; color: white; padding: 1px 15px 1px 5px;', id);
  console.log('%c query   ',  'background: #B3CC57; color: white; padding: 1px 15px 1px 5px;', query);
}
...
Template.ooInput.rendered = function () {
  var self = this;
  if (self.data.datetimepicker) {
    var pickerOptions = self.data.datetimepicker;
    var t = Template.instance();
    t.$('input').datetimepicker({
      pickDate: (pickerOptions === 'datetime') || (pickerOptions === 'date') ? true : false,
      pickTime: (pickerOptions === 'datetime') || (pickerOptions === 'time') ? true : false,
      minuteStepping: 5,
      icons: {
          time: " ico-clock",
          date: " ico-calendar",
          up: " ico-chevron-up",
          down: " ico-chevron-down"
      }
    });
  }
};
...
Template.ooMultiSelectButton.events({
  'click .js-openSelectModal': function() {
    ...
    if (self.agendaTime) {
      ...
      SelectModalOpen({activeIndex: self.activeIndex, templateName: self.templateModal, collection: self.collection, showResultsOnly: self.showResultsOnly, docId: self.docId, collectionParent: self.collectionParent})
    }
  }
})
