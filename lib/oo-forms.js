...
Template.ooInput.events({
  'input, blur' : function (e, t) {
    ...
    t.currentValue.set(UniHTML.purify(e.currentTarget.value));
...
Template.ooAutoInsertInput.events({
  ...
  'keypress .js-autoInsertInput': function(e, t) {
    ...
    if (e.keyCode === 13 && e.currentTarget.value.length > 0 && self.callback) {
      var fn = ooForms[self.callback];
      if (typeof fn === 'function') {
        fn(UniHTML.purify(e.currentTarget.value), self.docId);
...
Template.ooMultiSelectButton.events({
  'click .js-openSelectModal': function(e) {
    ...
    if (self.agendaTime) {
      ...
      selectModalOpen({activeIndex: self.activeIndex, templateName: self.templateModal, collection: self.collection, showResultsOnly: self.showResultsOnly, docId: self.docId, collectionParent: self.collectionParent, selectField: self.selectField,parentField: self.parentField, isInteractive:self.isInteractive})
    }
  }
})
