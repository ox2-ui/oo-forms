...
Template.ooTextarea.created = function() {
  ...
  self.scrollStart = new Blaze.ReactiveVar(0);
  self.inputId = new Blaze.ReactiveVar(Random.id());
  self.currentValue = new Blaze.ReactiveVar(self.data.value ? self.data.value: '');
  self.recordUpdateStatus = new Blaze.ReactiveVar(false);
  self.recordUpdateMessage = new Blaze.ReactiveVar(false);
...
Template.ooTextarea.events({
  'touchstart': function(e, t) {
    // sets start position
    t.scrollStart.set(e.originalEvent.touches[0].clientY)
  },
  'touchmove': function (e, t) {
    var tm = e.origisnalEvent.changedTouches[0].clientY;
    var scrollerPos = e.target.scrollTop;
    var scrollerMax = e.target.scrollHeight
    var scrollerLeft = e.target.clientHeight
    if (t.scrollStart.get() > tm) {
        if ((scrollerPos + scrollerLeft) === scrollerMax)
          e.preventDefault();
    } else {
      if (!scrollerPos)
        e.preventDefault();
...
Template.ooMonthSlider.helpers({
  dataMonths: function () {
    var templateInstance = Template.instance().view.parentView.parentView.parentView.parentView._templateInstance;
   return templateInstance.monthsArray.get()
  },
  scrollViewId : function () {
   return Template.instance().scrollViewId.get();
  },
  renderedHelper : function () {
    var t = Template.instance();
    Meteor.setTimeout(function(){
    scrollView = FView.byId(t.scrollViewId.get());
    Meteor.setTimeout(function(){
      var pageDone = scrollView.view.goToPage(t.currentIndex.get())
      console.log('%c pageDone   ',  'background: #5D76DB; color: white; padding: 1px 15px 1px 5px;', pageDone);
        // scrollView.view.goToPage(3)
      }, 500);

    t.autorun(function(){
      console.log('%c scrollView.properties.get   ',  'background: #5D76DB; color: white; padding: 1px 15px 1px 5px;');
      if (t.initialIndexSet.get()) {
      console.log('%c XX scrollView.properties.get(index)   ',  'background: #FF9900; color: white; padding: 1px 15px 1px 5px;', scrollView.properties.get('index'));

      } else {
        t.initialIndexSet.set(true)
      }
...
Template.ooMonthSlider.created = function () {
  ...
  self.scrollViewId = new Blaze.ReactiveVar(Random.id());
  self.initialIndexSet = new Blaze.ReactiveVar(false);
  self.scrollView = new Blaze.ReactiveVar(false);
  self.monthsArray = new Blaze.ReactiveVar(['December, 2014' ,'January, 2015', 'February, 2015', 'March, 2015', 'June, 2015', 'July, 2015', 'August, 2105', 'September, 2015', 'October, 2015', 'November, 2015', 'December, 2015'])
...
Template.ooMonthSlider.rendered = function () {
  console.log("%c Rendered:    ooMonthSlider    ",  "background: #2980b9; color: white; font-weight:bold; ");
};


Template.ooMonthSliderContent.rendered = function () {
  var self = this;
  var parentTmplInstance = self.view.parentView.parentView.parentView.parentView.parentView.parentView.parentView.parentView.parentView.parentView.parentView.parentView._templateInstance;
  // console.log('%c parentTmplInstance   ',  'background: #FF9900; color: white; padding: 1px 15px 1px 5px;', parentTmplInstance);
  // self.scrollView = FView.byId(self.scrollViewId.get()).view;
  // console.log('%c this   ',  'background: #FF9900; color: white; padding: 1px 15px 1px 5px;', this);
  // scrollView.properties.get('index')
  // self.scrollView.goToPage(self.data.currentItem)
  // console.log('%c scrollView   ',  'background: #FF9900; color: white; padding: 1px 15px 1px 5px;', self.scrollView);
  // self.scrollView.on('settle', function() {
  //     console.log('%c this  ff ',  'background: #FF9900; color: white; padding: 1px 15px 1px 5px;', this._cachedIndex);
  // });
};
