...
function updateRecord() {
  ...
  if (self.haveErrors.get()) {
    ...
    var query = {};
    if (self.data.field) {
      query[self.data.field] = self.saveAsNumbers ? Number(self.currentValue.get()) : self.currentValue.get();
    }
    if (self.data.fieldName) {
      query[self.data.fieldName] = self.saveAsNumbers ? Number(self.currentValue.get()) : self.currentValue.get();
    }
...
Template.ooAutoInsertInput.events({
  'click .js-addNewLocation' : function (e, t) {
    ...
    if (input && input.val() && self.callback) {
      var fn = ooForms[self.callback];
        if (typeof fn === 'function') {
          fn(input.val() ,self.docId, self.secondaryOptions ? self.secondaryOptions : '');
          input.val('');
        } else
          console.log('%c error: callback not found - ' + self.callback,  ' background: #BD4F7A; color: white; padding: 1px 15px 1px 5px;', self);
    }
  ...
  'click .js-openSelectModal': function(e) {
    ...
    if (self.agendaTime) {
      ...
      Collection.AgendaTimes.insert({
        ...
        starttime: getTimeFormatedInISO(moment(new Date())),
        endtime: getTimeFormatedInISO(moment(new Date())),
      ...
      selectModalOpen({activeIndex: self.activeIndex, templateName: self.templateModal, collection: self.collection, showResultsOnly: self.showResultsOnly, docId: self.docId, collectionParent: self.collectionParent, selectField: self.selectField,parentField: self.parentField, isInteractive:self.isInteractive, appId: self.appId, projectId: self.projectId})
    }
  }
})
