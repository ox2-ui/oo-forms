...
Template.ooFormButton.events({
  'click button': function(e, t) {
    ...
    for (var i = form.length - 1; i >= 0; i--) {
      ...
      if (!input) input = Blaze.getView(form[i]).parentView._templateInstance;

      input.validationCheck();
      if (input.haveErrors.get()) haveErrors = true;
    ...
    if (!haveErrors) {
      if (self.collection && self.formType) {
        if (self.formType === 'update' && self.docId) {
          ...
          Collection[self.collection].insert(query, function(err, res) {
            if (err) console.log(err);
          });
        } else {
          console.log(
            '%c error: incorrect formType specified - ' + self.formType,
            'background: #BD4F7A; color: white; padding: 1px 15px 1px 5px;',
            self,
          );
        }
        ...
        console.log(
          '%c self   ',
          'background: #B3CC57; color: white; padding: 1px 15px 1px 5px;',
          self,
        );
        var fn = ooForms[self.callback];

        if (typeof fn === 'function') fn(query, self.docId);
        else console.log(
            '%c error: callback not found - ' + self.callback,
            ' background: #BD4F7A; color: white; padding: 1px 15px 1px 5px;',
            self,
          );
      } else {
        console.log(
          '%c error: no update handlers passed  ',
          'background: #BD4F7A; color: white; padding: 1px 15px 1px 5px;',
          self,
        );
      }
...
function updateRecord() {
  ...
  if (self.haveErrors.get()) {
    console.log(
      '%c haveErrors   ',
      'background: #5D76DB; color: white; padding: 1px 15px 1px 5px;',
    );
    ...
    var query = {};
    if (self.data.field) {
      query[self.data.field] = self.saveAsNumbers
        ? Number(self.currentValue.get())
        : self.currentValue.get();
    }
    if (self.data.fieldName) {
      query[self.data.fieldName] = self.saveAsNumbers
        ? Number(self.currentValue.get())
        : self.currentValue.get();
    }
    // for tracking done fields
    if (self.data.checkmark) {
      if (self.data.field) query[self.data.field + 'Done'] = true;
      if (self.data.fieldName) query[self.data.fieldName + 'Done'] = true;
    }

    Collection[self.data.collection].update(
      { _id: self.data.docId },
      { $set: query },
      function(err, res) {
        if (res) {
...
Template.ooInput.onCreated(function() {
  var self = this;
  if (!self.data) {
    console.log(
      '%c ooInput no data passed to component   ',
      'background: #5D76DB; color: white; padding: 1px 15px 1px 5px;',
    );
    return;
  }
  ...
  self.inputId = new Blaze.ReactiveVar(
    self.data.inputId ? self.data.inputId : Random.id(),
  );
  self.currentValue = new Blaze.ReactiveVar(
    self.data.value ? self.data.value : '',
  );
  self.saveAsNumbers = self.data.safeAsNumbers ? true : false;
  ...
  self.noFormatting = self.data.noFormatting ? { noFormatting: true } : {};
  ...
  self.validationCheck = function() {
    ...
    if (
      self.noticeStatus.get() === 'notice:critical' ||
      self.charLimitStatus.get() === 'limit:critical'
    ) {
...
Template.ooInput.onRendered(function() {
  var self = this;
...
Template.ooInput.events({
  'input, blur': function(e, t) {
    ...
    if (self.autoResize) {
      el = e.target;
      if (
        el.scrollHeight > t.initialHeight.get() &&
        el.scrollHeight + 'px' !== el.style.height
      ) {
        el.style.height = 'auto';
        el.style.height = el.scrollHeight + 'px';
        el.scrollTop = el.scrollHeight;
  ...
  'touchmove .js-textarea': function(e, t) {
    ...
    if (self.multiline && !self.autoResize) {
      ...
      if (t.scrollStartY.get() > scrollMoveY) {
        if (topOffset + itemHeight === maxContentHeight) e.preventDefault();
      } else {
        // Prevent dragging when 'top' offset is 0 (content top reached)
        if (!topOffset) e.preventDefault();
      }
    }
  },
});
...
Template.ooAutoInsertInput.events({
  'click .js-addNewLocation': function(e, t) {
    var self = this, input = t.$('input');
    if (input && input.val() && self.callback) {
      var fn = ooForms[self.callback];
      if (typeof fn === 'function') {
        fn(
          input.val(),
          self.docId,
          self.secondaryOptions ? self.secondaryOptions : '',
        );
        input.val('');
      } else
        console.log(
  ...
  'keypress .js-autoInsertInput': function(e, t) {
    var self = this;
    if (e.keyCode === 13 && e.currentTarget.value.length > 0 && self.callback) {
      var fn = ooForms[self.callback];
      if (typeof fn === 'function') {
        fn(e.currentTarget.value, self.docId, self.secondaryOptions ? self.secondaryOptions : '');
...
Template.ooFormWrapper.created = function() {
  var self = this;
  if (!self.data.formId) self.data.formId = Random.id();
};
...
Template.ooTextarea.onCreated(function() {
  var self = this;
  if (!self.data) {
    console.log(
      '%c ooTextarea no data passed to component   ',
      'background: #5D76DB; color: white; padding: 1px 15px 1px 5px;',
    );
    return;
  }
  ...
  // Updating record with a debounce function to avoid firing too many times
  if (self.data.autoUpdate) self.lazyUpdate = _.debounce(updateRecord, 1000);
});
...
Template.ooToggle.events({
  'change input': function(e, t) {
    ...
    if (self.autoUpdate) {
      ...
      if (self.checkmark) query[self.field + 'Done'] = true;
            ...
            function(err, res) {
              if (err)
                ...
                Collection[self.collection].update(
                  { _id: self.docId },
                  { $set: { pictureId: res } },
                  function(errSecondary, resSecondary) {
                    if (errSecondary)
...
Template.ooSelect.helpers({
  selectName: function() {
    ...
    return record && record[self.fieldName]
      ? record[self.fieldName]
      : self.noRecordLabel ? self.noRecordLabel : 'No record set';
  },
});
...
Template.ooSelect.events({
  'click .js-openSelect': function() {
    var self = this;
    selectModalOpen({
      templateName: self.templateModal,
      collection: self.collection,
      showResultsOnly: self.showResultsOnly,
      docId: self.docId,
      collectionParent: self.collectionParent,
...
Template.ooMultiSelect.helpers({
  ...
  chechIfArray: function(itemData) {
...
Template.ooMonthSlider.created = function() {
  ...
  self.monthsArray = new Blaze.ReactiveVar([
    'January, 2015',
    'February, 2015',
    'March, 2015',
    'April, 2015',
  ...
  if (self.data && self.data.sessionName) {
    self.autorun(function() {
      ...
      Session.set(
        self.data.sessionName,
        self.data.momentFormat
          ? moment(newValue, 'MMMM, YYYY').format(self.data.momentFormat)
          : newValue,
      );
    });
  }
...
Template.ooMonthSlider.helpers({
  currentValue: function() {
    return Template.instance().monthsArray.get()[
